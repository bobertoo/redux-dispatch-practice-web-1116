...
describe('managePets reducer', function() {
  ...
  it("adds the pet when type is ADD_PET and the action has a payload property with the pet", function(){
    let state = {pets: [{name: 'Splash', type: 'turtle', id: 100}]}
    expect(managePets(state, {type: "ADD_PET", payload: {name: 'avalanche', species: 'puppy', id: 101}})).toEqual({pets: [{name: 'Splash', type: 'turtle', id: 100}, {name: 'avalanche', species: 'puppy', id: 101}]});
  })

  it("removes the pet when action type is REMOVE_PET and the action has a payload property of the id of pet to be removed", function(){
    let state = {pets: [{name: 'Splash', species: 'Turtle'}, {name: 'avalanche', species: 'puppy', id: 101}, {name: 'sally', species: 'camel', id: 102}]}
    expect(managePets(state, {type: "REMOVE_PET", payload: 101})).toEqual({pets: [{name: 'Splash', species: 'Turtle'}, {name: 'sally', species: 'camel', id: 102}]});
  })
  ...
  it("adheres to the rules of being a pure function, by not changing the original state, and instead returning a new object", function(){
    ...
    managePets(state, {type: "REMOVE_PET", payload: 101})
...
describe('render', function(){
  it('displays the state on the html', function(){
    global.document = jsdom.jsdom('<html><body><div id="container"></div></body></html>');
    dispatch({type: 'ADD_PET', payload: {name: 'Splash', species: 'Turtle', id: 100}})
    render()
...
describe('dispatch with render', function(){
  it('calls the render function which displays the updated html', function(){
    ...
    reducer.dispatch({type: 'ADD_PET', payload: {name: 'Splash', species: 'Turtle', id: 100}})
    let container = document.getElementById('container')
    expect(container.innerHTML).toEqual('<ul><li>Splash</li></ul>')
  })
})
